...
class DiskFS extends BaseFS {
  ...
  permissions(token, cb) {
    if (!this._config.api.requireAuth) {
      return cb()
    }

    debug('permissions', token)
    if (token === null || token === '') {
      return cb(new Error('You do not have access to perform this action.'))
    }

    postgres
      .query({text: 'SELECT * FROM auth WHERE token = $1', values: [token]})
      .then((res) => {
